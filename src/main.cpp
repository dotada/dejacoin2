...
#if defined(NDEBUG)
# error "Dejacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbba8217b3d9d1afaccef36386b274863dfeebe877b0f6888cb15b28c0e4596e6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dejacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dejacoin Signed Message:\n";
...
{
    // Dejacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dejacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dejacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Dejacoin launches 22/Sept/2022";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0434452c1f2d0bfac5de7a8aed794628c7e7beaf854751e0a2be9349634c7a8890938d7bbda850d37832b02ba7534e1418021cc7d99a7f44f7799c3e754b85ab56") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1663840890;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084695070;

        if (fTestNet)
        {
            block.nTime    = 1663840870;
            block.nNonce   = 386917988;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5b6604e29302c261c96f3f7bd9093fac2248e518550a9ac750fb7586abfc4ed1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xf3, 0xf3, 0xf3}; // Dejacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DejacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DejacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DejacoinMiner(CWallet *pwallet)
{
    printf("DejacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dejacoin-miner");
    ...
    try { loop {
        ...
        printf("Running DejacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DejacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DejacoinMiner, pwallet));
}
